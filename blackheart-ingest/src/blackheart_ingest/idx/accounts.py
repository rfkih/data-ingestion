...
``ResponseDto``, HS256 JWT in ``blackheart-token``), so the Papan app talks to this exactly as it would to the platform:
...
# ---- password policy (the platform's; the app mirrors it so the form can say it before the round trip)


def password_problem(pw: str) -> str | None:
    if not 12 <= len(pw) <= 100:
        return "Password must be between 12 and 100 characters"
