            ...
            for sym, ivl in instances:
                ...
                if run_id is not None:
                    try:
                        ...
                        # Trigger inference batch-predict if features were persisted
                        if written > 0:
                            try:
                                from ..webhooks import notify_inference_batch_ready
                                settings = get_settings()
                                result = asyncio.run(notify_inference_batch_ready(
                                    base_url=settings.inference_base_url,
                                    auth_token=settings.inference_auth_token.get_secret_value(),
                                    compute_run_id=str(run_id),
                                ))
                                logger.info("compute.inference_triggered", extra={
                                    "run_id": run_id, "result": result
                                })
                            except Exception as exc:  # noqa: BLE001
                                logger.warning("compute.inference_webhook_failed", extra={
                                    "error": repr(exc), "run_id": run_id
                                })
                                # Don't fail the compute job if webhook fails
                    except KeyboardInterrupt:
                        _fail_safely(
