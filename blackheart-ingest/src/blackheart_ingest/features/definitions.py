...
FEATURES: tuple[FeatureDef, ...] = (
    ...
    # ── FRED vol/credit macro axes (V180) — orthogonal crisis/credit signals.
    #    Raw in macro_raw via the FRED schedule; registry seeded in V180. ──────
    FeatureDef(
        name="gold_vix_level",
        version=1,
        family="macro",
        inputs=("GVZCLS",),
        transformer=_passthrough("GVZCLS"),
        pit_safe=True,
        ffill_policy="last_value",
        max_ffill_age_hours=72,
        description="CBOE Gold-ETF VIX (GVZCLS) daily close. Passthrough from FRED.",
    ),
    FeatureDef(
        name="gold_vix_zscore_252d",
        lookback_hours=9000,
        version=1,
        family="macro",
        inputs=("GVZCLS",),
        transformer=_rolling_zscore("GVZCLS", window=252, min_periods=60),
        pit_safe=True,
        ffill_policy="last_value",
        max_ffill_age_hours=72,
        description="252-business-day rolling z-score of gold-VIX (GVZCLS).",
    ),
    FeatureDef(
        name="oil_vix_level",
        version=1,
        family="macro",
        inputs=("OVXCLS",),
        transformer=_passthrough("OVXCLS"),
        pit_safe=True,
        ffill_policy="last_value",
        max_ffill_age_hours=72,
        description="CBOE Crude-Oil VIX (OVXCLS) daily close. Passthrough from FRED.",
    ),
    FeatureDef(
        name="oil_vix_zscore_252d",
        lookback_hours=9000,
        version=1,
        family="macro",
        inputs=("OVXCLS",),
        transformer=_rolling_zscore("OVXCLS", window=252, min_periods=60),
        pit_safe=True,
        ffill_policy="last_value",
        max_ffill_age_hours=72,
        description="252-business-day rolling z-score of oil-VIX (OVXCLS).",
    ),
    FeatureDef(
        name="vix_3m_level",
        version=1,
        family="macro",
        inputs=("VXVCLS",),
        transformer=_passthrough("VXVCLS"),
        pit_safe=True,
        ffill_policy="last_value",
        max_ffill_age_hours=72,
        description="CBOE 3-month VIX (VXVCLS) daily close. Passthrough from FRED.",
    ),
    FeatureDef(
        name="vix_3m_zscore_252d",
        lookback_hours=9000,
        version=1,
        family="macro",
        inputs=("VXVCLS",),
        transformer=_rolling_zscore("VXVCLS", window=252, min_periods=60),
        pit_safe=True,
        ffill_policy="last_value",
        max_ffill_age_hours=72,
        description="252-business-day rolling z-score of 3-month VIX (VXVCLS).",
    ),
    FeatureDef(
        name="hy_credit_spread_level",
        version=1,
        family="macro",
        inputs=("BAMLH0A0HYM2",),
        transformer=_passthrough("BAMLH0A0HYM2"),
        pit_safe=True,
        ffill_policy="last_value",
        max_ffill_age_hours=72,
        description="ICE BofA US High-Yield OAS (BAMLH0A0HYM2), level. Credit-stress proxy.",
    ),
    FeatureDef(
        name="hy_credit_spread_zscore_252d",
        lookback_hours=9000,
        version=1,
        family="macro",
        inputs=("BAMLH0A0HYM2",),
        transformer=_rolling_zscore("BAMLH0A0HYM2", window=252, min_periods=60),
        pit_safe=True,
        ffill_policy="last_value",
        max_ffill_age_hours=72,
